...
Uploads run on one background `URLSession`, so the transfer belongs to the system rather than the app process. The multipart body is written to Application Support, not tmp or caches, which the system may reclaim while the app is away. The ticket needed to finish the job, including the account it belongs to, rides along in the task description that the system persists. Versioned source tickets, response fragments and completed receipts are saved in a durable journal. A receipt is acknowledged only after its source revision is checked and the account's index is committed. Resumed uploads wait for matching system-owned transfers; an ambiguous server outcome is reconciled by the next checksum pass. Exporting still needs execution time. `BGProcessingTask` requests opportunistic windows for automatic backup and unfinished manual requests, using current network policy. Explicit cancellation clears a manual request; a processing-budget expiration retains it. iOS controls scheduling, and user force-quit requires reopening the app.
